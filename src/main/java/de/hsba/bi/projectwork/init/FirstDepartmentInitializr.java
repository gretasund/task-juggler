...
public class FirstDepartmentInitializr {
    ...
    public void firstProject() {
        ...
        // create tasks
        Task task1 = taskService.createNewTask(new Task(sandra, "Create a Company Page on Facebook", "In order to attract possible applicants our company has to be introduced on social media. A facebook page has to be created and filled with content.", 3, Task.Status.TESTING, "2020-08-28", project), project.getId());
        ...
        Task task2 = taskService.createNewTask(new Task(brigitte, "Create a Company Page on Instagram", "In order to attract possible applicants our company has to be introduced on social media. An instagram page has to be created an filled with content.", 3, Task.Status.DONE, null, project), project.getId());
        ...
        Task task4 = taskService.createNewTask(new Task(sandra, "Design an Employee-Survey", "We want to know how satisfied our current employees are and what we can do to make them even happier. To gain the necessary insights please plan a survey that will later be send to all employees.", 6, Task.Status.ACCEPTED, "2020-08-28", project), project.getId());
        taskService.setAssignee(task4.getId(), sandra);

        Task task5 = taskService.createNewTask(new Task(sandra, "Organize a Summer Party", "For staff retention reasons we will be holding a summer party again this year!", 6, Task.Status.ACCEPTED, "2020-10-20", project), project.getId());
        taskService.setAssignee(task5.getId(), sandra);

        Task task6 = taskService.createNewTask(new Task(conny, "Promote the Employee Referral Program", "95% of all applicants that apply for one of our jobs because of a referral are successful but only <1% of all employees know about the program. Please create a page in the intranet and promote it!", 3, Task.Status.ACCEPTED, "2020-09-10", project), project.getId());
    ...
    public void secondProject() {
        ...
        // create tasks
        Task task1 = taskService.createNewTask(new Task(sandra, "Find a good fitness club to cooperate with", "We want our employees to become member of a fitness club. In order to make this more attractive find a club we can cooperate with.", 3, Task.Status.ACCEPTED, "2020-12-28", project), project.getId());
        taskService.setAssignee(task1.getId(), sandra);
        bookingService.bookTime(task1.getId(), project.getId(), 1, "2020-06-28", sandra);
        bookingService.bookTime(task1.getId(), project.getId(), 4, "2020-06-30", sandra);

        Task task2 = taskService.createNewTask(new Task(brigitte, "Organize a company sports event", "Sport brings people together and is lots of fun. Organize a sports event", 3, Task.Status.DONE, null, project), project.getId());
        taskService.setAssignee(task2.getId(), brigitte);
        bookingService.bookTime(task2.getId(), project.getId(), 1, "2020-07-09", brigitte);
        bookingService.bookTime(task2.getId(), project.getId(), 1, "2020-07-11", brigitte);

        Task task3 = taskService.createNewTask(new Task(conny, "Find a good yoga teacher", "The latest employee survey found that 47% of our employees would like to do yoga at work. Let´s find a good yoga teacher.", 2, Task.Status.WORK_IN_PROGRESS, "2020-11-03", project), project.getId());
        taskService.setAssignee(task3.getId(), conny);
        bookingService.bookTime(task3.getId(), project.getId(), 3, "2020-07-09", student);

        Task task4 = taskService.createNewTask(new Task(sandra, "Department breakfasts", "Each department has the possibility to have breakfast together (once a month). Promote this.", 6, Task.Status.ACCEPTED, "2021-02-28", project), project.getId());
        taskService.setAssignee(task4.getId(), sandra);

        Task task5 = taskService.createNewTask(new Task(student, "Partner health insurance", "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.", 6, Task.Status.ACCEPTED, "2020-10-20", project), project.getId());
